...
<section class="container">
    ...
    <br><br>
    <form:form action="/admin/cua-hang/add" method="post" modelAttribute="cuaHang">
        <label>Tên: </label>
        <form:input path="ten"/>
    ...
    <table class="table">
        ...
        <c:forEach items="${listCuaHang}" var="cuaHang" varStatus="i">
            <tr>
                ...
                <td>${cuaHang.trangThai==0?"Đang kinh doanh":"Ngưng kinh doanh"}</td>
                <td>
                    <button class="btn btn-primary"><a href="/admin/cua-hang/detail/${cuaHang.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a href="/admin/cua-hang/view-update/${cuaHang.id}">Sửa</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a href="/admin/cua-hang/remove/${cuaHang.id}"
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/cua-hang/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/cua-hang/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/cua-hang/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
