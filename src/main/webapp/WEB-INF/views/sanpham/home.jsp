...
<section class="container">
    ...
    <br><br>
    <form:form action="/admin/san-pham/add" method="post" modelAttribute="sanPham">
        <label>Tên: </label>
        <form:input path="ten"/>
    ...
    <table class="table">
        ...
        <c:forEach items="${listSanPham}" var="sanPham" varStatus="i">
            <tr>
                ...
                <td>${sanPham.ma}</td>
                <td>${sanPham.ten}</td>
                <td>${sanPham.trangThai==0?"Đang kinh doanh":"Ngừng kinh doanh"}</td>
                <td>
                    <button class="btn btn-primary"><a href="/admin/san-pham/detail/${sanPham.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a href="/admin/san-pham/view-update/${sanPham.id}">Update</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a href="/admin/san-pham/remove/${sanPham.id}"
                                                      onclick="return thongBao('xóa sản phẩm')">Remove</a>
                    </button>
                </td>
            </tr>
        </c:forEach>
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/san-pham/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/san-pham/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/san-pham/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
