...
<section class="container">
    ...
    <br><br>
    <form:form action="/admin/chuc-vu/add" method="post" modelAttribute="chucVu">
    ...
    <table class="table">
        ...
        <c:forEach items="${listChucVu}" var="chucVu" varStatus="i">
            <tr>
                ...
                <td>${chucVu.ma}</td>
                <td>${chucVu.ten}</td>
                <td>${chucVu.trangThai==0?"Còn sử dụng":"Ngưng sử dụng"}</td>
                <td>
                    <button class="btn btn-primary"><a href="/admin/chuc-vu/detail/${chucVu.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a href="/admin/chuc-vu/view-update/${chucVu.id}">Sửa</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a href="/admin/chuc-vu/remove/${chucVu.id}"
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/chuc-vu/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/chuc-vu/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/chuc-vu/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
