...
<section class="container">
    ...
    <form:form action="/admin/nhan-vien/add" method="post" modelAttribute="nhanVien">
        ...
        <form:radiobutton path="gioiTinh" checked="true" value="Nam" label="Nam"/>
        <form:radiobutton path="gioiTinh" value="Nữ" label="Nữ"/>
        <br><br>
        <label>Ngày sinh:</label>
        <form:input type="date" path="ngaySinh"/>
        <form:errors path="ngaySinh" cssStyle="color:red" cssClass="error"/>
        <br><br>
        ...
        <form:select path="cuaHang" id="selectCuaHang" onchange="getData()">
            <c:forEach items="${listCuaHang}" var="cuaHang">
                <form:option value="${cuaHang.id}" data-ten="${cuaHang.ten}" label="${cuaHang.ma}"/>
            </c:forEach>
        </form:select>
        <label id="tenCuaHang"></label>
        <br><br>
        <label>Mã chức vụ:</label>
        <form:select path="chucVu" id="selectChucVu" onchange="getData()">
            <c:forEach items="${listChucVu}" var="chucVu">
                <form:option value="${chucVu.id}" data-ten="${chucVu.ten}" label="${chucVu.ma}"/>
            </c:forEach>
        </form:select>
    ...
    <table class="table">
        ...
        <c:forEach items="${listNhanVien}" var="nhanVien" varStatus="i">
            <tr>
                ...
                <td>
                    <button class="btn btn-primary"><a href="/admin/nhan-vien/detail/${nhanVien.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a
                            href="/admin/nhan-vien/view-update/${nhanVien.id}">Sửa</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a href="/admin/nhan-vien/remove/${nhanVien.id}"
                                                      onclick="return thongBao('cho xóa')">Xóa</a>
                    </button>
                </td>
            </tr>
        </c:forEach>
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/nhan-vien/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/nhan-vien/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/nhan-vien/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
...
<script>
    ...
    function getData() {
//label cuahang
        var selectCuaHang = document.getElementById("selectCuaHang");
        var indexCuaHang = selectCuaHang.selectedIndex;
        var optionsCuaHang = selectCuaHang.options[indexCuaHang];
        document.getElementById("tenCuaHang").innerHTML = optionsCuaHang.dataset.ten;
// label chucVu
        var selectChucVu = document.getElementById("selectChucVu");
        var indexChucVu = selectChucVu.selectedIndex;
        var optionsChucVu = selectChucVu.options[indexChucVu];
        document.getElementById("tenChucVu").innerHTML = optionsChucVu.dataset.ten;
    }
