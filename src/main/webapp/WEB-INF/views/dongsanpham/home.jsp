...
<section class="container">
    ...
    <br><br>
    <form:form action="/admin/dong-san-pham/add" method="post" modelAttribute="dongSanPham">
        <label>Tên: </label>
        <form:input path="ten"/>
    ...
    <table class="table">
        ...
        <tbody>
        <c:forEach items="${listDongSp}" var="dongSanPham" varStatus="i">
            <tr>
                <th scope="row">${i.index+1}</th>
                <td>${dongSanPham.ma}</td>
                <td>${dongSanPham.ten}</td>
                <td>${dongSanPham.trangThai==0?"Đang kinh doanh":"Ngừng kinh doanh"}</td>
                <td>
                    <button class="btn btn-primary"><a href="/admin/dong-san-pham/detail/${dongSanPham.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a href="/admin/dong-san-pham/view-update/${dongSanPham.id}">Sửa</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a href="/admin/dong-san-pham/remove/${dongSanPham.id}"
                                                      onclick="return thongBao('xóa dòng sản phẩm')">Xóa</a>
                    </button>
                </td>
            </tr>
        </c:forEach>
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/dong-san-pham/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/dong-san-pham/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/dong-san-pham/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
