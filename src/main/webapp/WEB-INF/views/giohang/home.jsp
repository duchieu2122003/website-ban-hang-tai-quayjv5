...
<section class="container">
    ...
    <br>
    <form:form action="/admin/gio-hang/add" method="post" modelAttribute="gioHang">
    ...
    <table class="table">
        ...
        <c:forEach items="${listGioHang}" var="gioHang" varStatus="i">
            <tr>
                ...
                <td>${gioHang.ma}</td>
                <td>${gioHang.khachHang.ma} ${gioHang.khachHang.ten}</td>
                <td>${gioHang.ngayTao}</td>
                <td>${gioHang.ngayThanhToan}</td>
                <td>${gioHang.tenNguoiNhan}</td>
                <td>${gioHang.sdt}</td>
                <td>${gioHang.diaChi}</td>
                <td>${gioHang.trangThai==0?"Đã thanh toán":"Chưa thanh toán"}</td>
                <td>
                    <button class="btn btn-primary"><a href="/admin/gio-hang/detail/${gioHang.id}">Xem</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-warning"><a
                            href="/admin/gio-hang/view-update/${gioHang.id}">Sửa</a>
                    </button>
                </td>
                <td>
                    <button class="btn btn-danger"><a
                            href="/admin/gio-hang/remove/${gioHang.id}">Xóa</a>
                    </button>
                </td>
            </tr>
        </c:forEach>
        ...
        <ul class="pagination">
            <c:if test="${currentPage >1}">
                <li class="page-item" onclick="return thongBao('previous')">
                    <a class="page-link" href="/admin/gio-hang/hien-thi?pageNo=${currentPage - 1}">Previous</a>
                </li>
            </c:if>
            <c:forEach begin="1" end="${totalPages}" varStatus="i">
                <li class="page-item" onclick="return thongBao('tới trang ${i.index}')">
                    <a class="page-link" href="/admin/gio-hang/hien-thi?pageNo=${i.index}">${i.index}</a></li>
            </c:forEach>
            <c:if test="${currentPage < totalPages}">
                <li class="page-item" onclick="return thongBao('next')">
                    <a class="page-link" href="/admin/gio-hang/hien-thi?pageNo=${currentPage + 1}">Next</a>
                </li>
            </c:if>
        </ul>
    </nav>
